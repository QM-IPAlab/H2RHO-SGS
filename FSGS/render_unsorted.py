# render image name is different from the .npz name
...
def render_video(source_path, model_path, iteration, views, gaussians, pipeline, background, fps=30):
    ...
    render_poses = []
    # folder_path = os.path.join(source_path,'npz_1')
    folder_path = os.path.join(source_path,'npz-0108')

    # # 遍历指定目录下的所有文件
    for item in os.listdir(folder_path):
        # print(f'item: {item} \n')
        item_path = os.path.join(folder_path,item)
        render_poses_onepath = []
        for file_name in sorted(os.listdir(item_path)):
            if file_name.endswith('.npy'):  # 只处理 .npy 文件
                ...
                render_poses_onepath.append(data)
        render_poses.append(render_poses_onepath)
    ...
    for idj, render_pose in enumerate(render_poses):
        render_path_j = os.path.join(render_path,str(idj))
        os.makedirs(render_path_j,exist_ok = True)
        print(f'reaching route number:{idj}')
        for idx, pose_c2w in enumerate(tqdm(render_pose, desc="Rendering progress")):
            R_s = pose_c2w[:3, :3]
            t_s = pose_c2w[:3, 3]
            ...
            torchvision.utils.save_image(img, os.path.join(render_path_j, '{0:05d}'.format(idx) + ".png"))
