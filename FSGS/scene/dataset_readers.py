...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping):
    ...
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        # bounds = np.load(os.path.join(path, 'poses_bounds.npy'))[idx, -2:]  # 是该相机的close_depth, inf_depth[ 3.63105887, 61.02630758]
        bounds = np.array([0,0])
...
def readKKInfo(path, images, eval, n_views=0, llffhold=7):
    '''
    path:args.sourth_path  dataset/mipnerf360/kitchen_img8
    images:args.images  default is _images='images_8'
    eval: args.eval 
    n_views: args.n_views int
    '''
    ...
    rgb_mapping = [f for f in sorted(glob.glob(os.path.join(path, reading_dir, '*')))
                   if f.endswith('JPG') or f.endswith('jpg') or f.endswith('png')]
    cam_extrinsics = {cam_extrinsics[k].name: cam_extrinsics[k] for k in cam_extrinsics}
    # need poses_bounds.npy
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,
                             images_folder=os.path.join(path, reading_dir),  path=path, rgb_mapping=rgb_mapping)
    # cam_infos_unsorted = readColmapCameras2(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,images_folder=os.path.join(path, reading_dir))
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
