...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        """
        self.model_path = args.model_path  #output/kitchen_onlyimage8
        ...
        # read colmap-format dataset, get scene_info
        if args.kk:
            scene_info = sceneLoadTypeCallbacks["kk"](args.source_path, args.images, args.eval, args.n_views)
        ...
        # 这段代码的主要目的是加载训练和测试相机信息，并根据不同的场景生成伪相机（pseudo_cams）。下面是对代码逐行的详细解释
        for resolution_scale in resolution_scales:
            ...
            # generate_random_poses_llff 返回的是围绕某个焦点的随机螺旋路径，更适合用于 LLFF 数据集类型的渲染任务。需要之前相机的bounds(最近深度 最远深度)
            # generate_random_poses_360 返回的是围绕一个中心点均匀分布的椭圆路径，更适合用于360度全景场景。
            pseudo_cams = []
            if args.source_path.find('360'):
