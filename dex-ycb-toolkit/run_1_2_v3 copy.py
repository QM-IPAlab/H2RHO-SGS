#multiprocess for /home/e/eez095/dexycb_data/20200813-subject-02
import subprocess
import os
import re
import argparse
...
def execute_commands(dataSet, dataset_path, completed_file, error_file):
    res = True
    common_commands = [
        f"python -W ignore examples/get_pointcloud.py --name {dataSet}"
    ]
    frame_specific_command = "python -W ignore examples/visualize_pose.py --src {}"
    ...
    for data_frame in data_frames:
        if not re.match(r'\d+_frame$', os.path.basename(data_frame)):
            continue
        if os.path.join(dataset_path, data_frame) in completed_datasets:
            continue
        cmd_f = frame_specific_command.format(data_frame)
        print(f"{cmd_f}")
        try:
            subprocess.run(cmd_f, shell=True, check=True)
            with open(completed_file, 'a') as cf:
                cf.write(f"{os.path.join(dataset_path, data_frame)}\n")
        except subprocess.CalledProcessError as e:
            error_details = traceback.format_exc()
            with open(error_file, 'a') as ef:
                ef.write(f"Error executing command: {cmd_f}\nError message: {e} {error_details}\n\n")
            res = False
    with open(completed_file, 'a') as cf:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--completed_file', type=str, default='/home/e/eez095/project/dex-ycb-toolkit/log/completed_datasets.txt', help="File to store completed dataset paths")
    parser.add_argument('--error_file', type=str, default='/home/e/eez095/project/dex-ycb-toolkit/log/error_log.txt', help="File to store error messages")
