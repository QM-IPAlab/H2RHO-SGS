...
class DexYCBDataset():
  ...
  def __init__(self, setup, split):
    ...
    self._subjects = [_SUBJECTS[i] for i in range(len(_SUBJECTS)) ]
    ...
    offset = 0
    # if True: 
    #   n = '20200813-subject-02/20200813_145341'
    # import pdb 
    # pdb.set_trace()
    for n in self._subjects:  #select data
      seq = sorted(os.listdir(os.path.join(self._data_dir, n)))
      seq = [os.path.join(n, s) for s in seq]
      assert len(seq) == 100 # /home/e/eez095/dexycb_data/20200813-subject-02/里有100个文件夹
