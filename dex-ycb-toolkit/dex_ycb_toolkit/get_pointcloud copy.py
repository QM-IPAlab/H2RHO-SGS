...
class SequenceLoader1():
  ...
  def __init__(
    ...
    for i in tqdm(range(self._num_frames),desc='generating mask pointcloud'): # 某一时刻 i_frme文件夹里
        ...
        self.max_depth_threshold = 10000000000
        self.depth_normal = 100
        ...
        for c in range(self._num_cameras):
          ...
          # expanded_mask = np.expand_dims(valid_mask, axis=-1)  # 在最后一个维度增加一个维度
          # pcl_valid_mask = np.repeat(expanded_mask, repeats=3, axis=-1)
       
          # # pcl_valid_mask (480, 640, 3)/ pcl_valid_mask  self._color_frame[c]
          # # filtered_points = np.where(pcl_valid_mask, p_mask, np.nan)
          # # filtered_points = filtered_points[~np.isnan(filtered_points).any(axis=(1,2))]
          # filtered_points = p_mask[pcl_valid_mask]
          # filtered_colors = self._color_frame[c][pcl_valid_mask]
        
          # filtered_colors = np.where(pcl_valid_mask,self._color_frame[c],np.nan) 
          # filtered_colors = filtered_colors[~np.isnan(filtered_colors).any(axis=(1,2))]
        ...
        # pcl_mask_object_hand = np.concatenate([np.array(mask) for mask in self._pcd_mask_hand_object], axis=0)
        # pcl_color = np.concatenate([np.array(color) for color in self._color_pcd_mask_hand_object], axis=0)
        # pcl_valid_mask = np.array(self.valid_handover_mask)
        
        pcl_mask_object_hand = pcl_mask_object_hand.reshape(-1, 3)
        pcl_color = pcl_color.reshape(-1,3)
        pcl_mask_3d = pcl_mask_3d.reshape(-1,3)

        # pcl_mask_object_hand = pcl_mask_object_hand[pcl_mask_3d]
        # pcl_color = pcl_color[pcl_mask_3d]

        # pcl_valid_mask = pcl_valid_mask.reshape(-1,1)
        import pdb 
        pdb.set_trace()
        pcl_valid_mask = pcl_mask_object_hand[:, 2] <= self.depth_normal  # 假设 Z 值是深度
        pcl_mask_object_hand = pcl_mask_object_hand[pcl_valid_mask]
        pcl_color = pcl_color[pcl_valid_mask]
        # 保存点云为 PLY 文件
        import trimesh
  ...
  def _deproject_depth_and_mask_hand_objct(self, d,color, c, l):
    ...
    valid_mask = np.ones((self._h, self._w), dtype=bool)
