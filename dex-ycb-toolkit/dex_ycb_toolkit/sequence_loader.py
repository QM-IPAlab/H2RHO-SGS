...
class SequenceLoader():
  ...
  def __init__(
    ...
    if self._preload:
      ...
      # self._pcd_vert:(8,72, 480, 640, 3) 这里得到的是8个相机，72帧的，所有点云
      ...
      # import trimesh
      # p = p.reshape(-1, 3)
      # point_cloud = trimesh.PointCloud(p)
      # # 保存点云为 PLY 文件
      ...
      # P_m = np.array(P_m) 
      # P_m = P_m.reshape(-1,3)
      # point_cloud_m = trimesh.PointCloud(P_m)
      # # 保存点云为 PLY 文件
      # point_cloud_m.export('./mask_output.ply')
      # import pdb 
      # pdb.set_trace()
  ...
  def _deproject_depth_and_filter_points(self, d, c):
    ...
    """
    nd = d.ndim
    d = d.astype(np.float32) / 1000 #单位转换为毫米
  ...
  def pcd_vert(self):
    if self._preload:
      import pdb 
      pdb.set_trace() 
      #self._pcd_ver (8, 72, 480, 640, 3) 72帧 八个相机 所有点云 -> (8,480,640,3)某一帧 八个相机 所有点云
  ...
  @property
  def download_pcd_vert(self):
    if self._preload:
      import trimesh
      for i in range(self._num_frames):
        pcl = [x[i] for x in self._pcd_vert]
        print(len(pcl))
        np_pcl = np.array(pcl,copy=True)
        #  p ( 480, 640, 3)
        # self._pcd_vert list (8,480,640,3)
        
        np_pcl = np_pcl.reshape(-1, 3)
        print(np_pcl)
        point_cloud = trimesh.PointCloud(np_pcl)
        # 保存点云为 PLY 文件
        ply_output_path = os.path.join(os.path.join(self._dex_ycb_dir, self._name), str(i)+ '_points3D.ply')
