...
import numpy as np
import cv2
os.environ["PYOPENGL_PLATFORM"] = "egl" #告诉PyOpenGL 使用 EGL 后端进行渲染。这在没有传统窗口系统的无头环境中尤其有用。
from dex_ycb_toolkit.get_pointcloud import SequenceLoader1
...
class Renderer():
  ...
  def __init__(self, name, device='cuda:0'):
    ...
    self._loader = SequenceLoader1(self._name,
                                  device=device,
                                  preload=True,
                                  app='renderer')
    # now get the scene pointcloud
    ...
    # Create meshes for YCB objects. 单独物体的点云
    self._mesh_y = []
