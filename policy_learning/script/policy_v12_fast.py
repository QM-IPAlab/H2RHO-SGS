...
class CustomDataset(Dataset):
    def __init__(self, txt_path, transform=None, save_txt_path=None):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            # results = list(tqdm(executor.map(self.process_scene, scene_paths), total=len(scene_paths), desc="Loading scenes"))
            results = list(tqdm(executor.map(self.safe_process_scene, scene_paths), total=len(scene_paths), desc="Loading scenes"))
    
        # 过滤掉失败的结果（返回 None 的）
        results = [result for result in results if result is not None]
        for scene_data in results:
            self.data.extend(scene_data)


    def safe_process_scene(self,scene_path):
        try:
            res = self.process_scene(scene_path)
            return res  # 确保这里是实例方法或者静态方法
    ...
    def process_scene(self, data_path):
        """ 处理单个 scene 目录 """
        image_dir = os.path.join(data_path, "FSGS_output", "video", "sample_v510000")
        trajectory_dir = os.path.join(data_path, "trajectory_v5")
        ...
        for subfolder in subfolders:
            ...
            for i in range(len(img_files) - 1):
                ...
                # print(f'{os.path.join(img_folder, img_files[i])}\n {os.path.join(traj_folder, traj_files[i])}\n {next_traj_path}\n {hand_img}\n{object_img}\n{pre_grasp_class}\n\n')
                scene_data.append({
    ...
    def __getitem__(self, idx):
        ...
        # **优化：使用 joblib 并行加载图像**
        img = joblib.delayed(Image.open)(data_item["img_path"]).convert("RGB")

        # **优化：避免 transform 变慢**
        if self.transform:
            img = self.transform(img, data_item["hand_mask"], data_item["object_mask"])

        # **优化：使用 numpy.memmap 预加载大数据**
        current_traj = np.load(data_item["current_traj"], mmap_mode='r')
        next_traj = np.load(data_item["next_traj"], mmap_mode='r')
