# Ltrans Lrot Lclass: score1:score2:1 
# use cos loss for rot
...
score_t = 1000
score_r = 90*score_t#ltrans:Lclass
...
def cosine_loss(pred, gt):
    pred_norm = F.normalize(pred, dim=-1)  # 归一化预测值
    gt_norm = F.normalize(gt, dim=-1)      # 归一化真实值
    cosine_similarity = (pred_norm * gt_norm).sum(dim=-1)  # 计算点积
    return (1 - cosine_similarity).mean()  # 计算损失
...
class PoseLoss(nn.Module):
    ...
    def forward(self, pred_pose, pred_class, target_pose, target_class):
        ...
        alpha = 0.5
        # 计算归一化后的 MSE  +co s损失
        rot_loss = alpha * self.mse_loss(pred_rot, target_rot) + (1-alpha)*cosine_loss(pred_rot, target_rot)
        trans_loss = alpha * self.mse_loss(pred_trans, target_trans) + (1-alpha)*cosine_loss(pred_trans, target_trans)

        # print(pred_class.dtype,target_class.dtype)
        class_loss = custom_bce_loss(pred_class, target_class)
        
        # 总损失 = 旋转损失 + 位移损失
        total_loss = score_r*rot_loss + score_t*trans_loss + class_loss
        return total_loss, rot_loss, trans_loss, class_loss

# Training and evaluation function with mixed precision
def train_and_evaluate(model, train_dataloader, test_dataloader, criterion, optimizer, epochs=10, device='cuda',model_path='./best_pose_estimation_model_v10_0209.pth',log_file ='./train_log.txt'):
    model.to(device)
...
# Evaluate function
def evaluate(model, test_dataloader, criterion, device='cuda',mode='train', log_file='train_v14.txt'):
...
def main(mode, txt_path, save_txt_path, test_image_path, evaluate_txt_path, model_path, test_traj_path, next_pose_path,hand_ply_file,obj_ply_file,mask_dir,log_file):
    model = PoseEstimationModel()
    # Multi-GPU support
    ...
    if mode == 'train':
        ...
        train_and_evaluate(model, train_dataloader, test_dataloader, criterion, optimizer, epochs=50, device=device,model_path = model_path,log_file=log_file)
        ...
        gt_rot_poses, gt_trans_poses, pred_rot_poses, pred_trans_poses,gt_label,predict_label= evaluate(model, evaluate_dataloader, criterion, device='cuda',mode = 'evaluate', log_file = log_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_file',type=str,help='training log')

    args = parser.parse_args()

    main(args.mode, args.txt_path, args.save_txt_path, args.test_image_path, args.evaluate_txt_path, args.model_path, args.test_traj_path,args.next_pose_path,args.hand_ply_file,args.obj_ply_file,args.mask_dir,args.log_file)
