...
def draw_scene(pc,
    ...
    if grasp_scores is not None:
        ...
        top1 = np.array(grasp_scores).argsort()[-1:][::-1]
        sorted_scores = np.sort(grasp_scores)[::-1]  # Sort in descending order
        ...
        third_largese_index = np.where(grasp_scores == third_largest_score)[0][0]
        print(min_score, max_score, top1)
