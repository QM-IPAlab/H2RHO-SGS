...
def getSafeGrasps(grasps, pc_h):
    y_min, y_max = -0.008, 0.015 #-0.01, 0.01
    ...
    for i in range(grasps.shape[0]):
        grasp = grasps[i]
         # 将手部点云从世界坐标系转换到抓取坐标系 但是这个函数返回的只是有效抓取的索引，不会将原有的抓取转换标标系的
        ...
        for j in range(pc_h_grasp.shape[0]):
            if np.linalg.norm(pc_h_grasp[j])<0.12:
                in_collision = True
                break
        if not in_collision:
            safe_list.append(i)
...
class GraspEstimator:
    ...
    def generate_and_refine_grasps_safe(
        ...
        inlier_indices = utils.get_inlier_grasp_indices(grasps_list,
                                                        torch.zeros(1, 3).to(
                                                            self.device),
                                                        threshold=1.0,
                                                        device=self.device)# This function returns all grasps whose distance between the mid of the finger tips and the query point is less than the threshold value.
        self.keep_inliers(grasps_list, confidence_list, z_list, pc_list,
                          inlier_indices)
    ...
    def refine_grasps(self, pc, grasps, refine_method, num_refine_steps=10, min_grasps=10000, min_refine_step=5):
        # 对给定的抓取姿势进行优化，以提高抓取成功的概率。该函数通过不同的优化方法（如基于梯度或基于采样的方法）来调整抓取姿势，并在每个迭代步骤中评估抓取的有效性
        # num_refine_steps：指定优化迭代的步数，默认为 10。
        # min_grasps：在优化过程中，要求至少有多少个成功的抓取姿势。
        # min_refine_step：最小迭代步数，确保在达到成功数量之前不会提前结束
